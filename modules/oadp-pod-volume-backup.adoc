...
Velero supports two approaches for determining pod volumes. Use the opt-in or opt-out approach to allow Velero to decide between an FSB, a volume snapshot, or a Data Mover backup. 

* *Opt-in approach*: With the opt-in approach, volumes will be backed up using FSB even though the backup spec is using snapshot or DataMover as the default option. Opted-in by using annotations, use Kopia or Restic. 

* *Opt-out approach*: With the opt-out approach, volumes are not backed up using FSB. Volumes, opted out by the addition of annotations, are backed up using a snapshot or Data Mover.
